...
const InvoiceDetail: React.FC<InvoiceDetailProps> = ({ invoice, onBack }) => {
  ...
  // 2️⃣ Calcular impuestos
  const calculatedTax = invoice.taxAmount ?? (calculatedSubtotal * (invoice.taxRate ?? 0));

  // 3️⃣ Calcular total
  const calculatedTotal = calculatedSubtotal + calculatedTax;

  // Generar QR con total correcto
  useEffect(() => {
    const generateQr = async () => {
      if (!invoice?.id) return;
      const qrValue = `Factura ID: ${invoice.id} | Cliente: ${invoice.requesterName} | Total: Bs. ${calculatedTotal.toFixed(2)}`;
      const dataUrl = await QRCode.toDataURL(qrValue, { width: 120, margin: 1 });
      setQrDataUrl(dataUrl);
    };
    generateQr();
  }, [invoice, calculatedTotal]); // siempre mantener la misma cantidad de dependencias
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      doc.text(`${calculatedTax.toFixed(2)}`, totalX, y, { align: 'right' });
      y += 8;
      ...
      doc.text(`Bs. ${calculatedTotal.toFixed(2)}`, totalX, y, { align: 'right' });
          ...
          <div className="flex justify-center pt-4">
            <div className="w-full sm:w-96 space-y-3 p-4 bg-indigo-50 rounded-xl shadow-inner flex flex-col items-center">
              ...
              <div className="flex justify-between w-full border-t pt-3 border-indigo-200">
                ...
                <span className="text-xl font-extrabold text-indigo-600">{calculatedTotal.toFixed(2)}</span>
              </div>
